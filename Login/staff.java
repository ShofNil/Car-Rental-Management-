...
public class staff implements ActionListener {
    ...
    public staff() {
        frame = new JFrame("Staff Registration");
        ...
        JLabel staffLabel = new JLabel("STAFF REGISTRATION");
        staffLabel.setBounds(100, 70, 350, 30);
        ...
        loginLabel.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                frame.dispose();
                new llogin(); // Assuming llogin is the login class
            }
        });
    ...
    private void register() {
        ...
        UserStaff newUser  = new UserStaff(name, email, password); // Assuming UserStaff is defined

        try {
            FileWriter writer = new FileWriter("userStaffdata.txt", true);
            writer.write(newUser .getName() + "," + newUser .getEmail() + "," + newUser .getPassword() + "\n");
            writer.close();

            // Create the empty file for the new user
            String filePath = newUser .getName() + "_assigned_tasks.txt";
            File assignedTasksFile = new File(filePath);
            assignedTasksFile.createNewFile(); // Creates the file without writing anything
